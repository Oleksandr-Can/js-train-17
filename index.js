...
function countEmails(str) {
  ...
  const numberOfEmails = arr.length;
...
function findWordOccurrences(str, word) {
  ...
  // const arr = [...str.matchAll(regex)];
  // const matches = arr.map(match => match.index);
  // Повертаємо масив
  return matches;
}
...
const result = regex.flags;
  ...
  return result.includes('g') && result.includes('m');
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const result = str.replaceAll(regex, newWord)
